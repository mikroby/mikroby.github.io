...
const sensors = ['accelerometer', 'magnetometer', 'gyroscope', 'ambient-light-sensor']
const sensorObject = {
  'magnetometer'() { return new Magnetometer({ frequency: 10 }) }
  ,
  'accelerometer'() { return new Accelerometer({ frequency: 10 }) }
  ,
  'gyroscope'() { return new Gyroscope() }
  ,
  'ambient-light-sensor'() { return new AmbientLightSensor() }
}
...
const useSensor = (sensorName, index, result) => {
  ...
  if (sensorName === 'ambient-light-sensor') {
    sensor.addEventListener('reading', () => {
      xValue.textContent = sensor.illuminance      
    })
  } else {
    sensor.addEventListener('reading', () => {
