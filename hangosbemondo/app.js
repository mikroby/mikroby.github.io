...
const playSound = (event) => {
  ...
  if (form.button.value === 'start') {
    ...
    signal.play()
    const [trainType, verb, time, track] = Object.values(form).map(({ name }) => form[name].value)
    const hour = Number(time.slice(0, 2))
    const minute = Number(time.slice(3, 5))

    message.text =
      `${trainType} ${verb} ${hour} óra ${minute} perckor ${track === '2.' || track === 'Bé' ? 'a' : 'az'} ${track} ${verb === 'indul' ? 'vágányról' : 'vágányra'}`
  } else {
